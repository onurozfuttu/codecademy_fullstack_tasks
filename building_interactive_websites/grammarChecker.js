...
const completedWords= filteredWords.map(word=>{
  if(word===misspelledWord){
    return 'beatiful'
  }else return word;
})

const badWordIndex = completedWords.findIndex(word=>{
  return word===badWord;
})
//console.log(completedWords[78]);

//console.log(badWordIndex);
completedWords[78] = 'really';
//console.log(completedWords[78]);

const lengthCheck = completedWords.every(word=>{
  return word < 10;
})
